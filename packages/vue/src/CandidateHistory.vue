  ...
  <div class="flex flex-col gap-3">
    <div class="flex flex-col gap-1.5">
      ...
      <div class="p-3 rounded-small bg-bg-gray1">
        <p class="text-body2 text-gray800">{{ aiSummary }}</p>
      </div>
    </div>
    ...
    <div v-if="latest" ref="wrapRef" class="relative flex items-center gap-4">
      ...
      <div class="min-w-0 flex-1 flex items-center gap-1">
        <span class="text-body2 font-medium text-gray800 truncate">{{ latest.company }}</span>
        <span class="text-body2 font-medium text-gray700 truncate">{{ latest.role }}</span>
      ...
      <CareerTooltip v-if="open" :careers="careers" class="absolute right-0 top-full mt-1 z-10" />
    </div>

    <div v-if="education" class="flex items-center gap-4">
      <span class="shrink-0 text-body2 font-medium text-gray700">학력</span>
      <div class="min-w-0 flex-1 flex items-center gap-1">
        <span class="text-body2 font-medium text-gray800 truncate">{{ education.school }}</span>
        <span class="text-body2 font-medium text-gray800">{{ education.degree }}</span>
        <span class="text-body2 font-medium text-gray700 truncate">{{ education.major }}</span>
        <span class="shrink-0 text-detail text-gray600">{{ education.period }}</span>
        <span class="shrink-0 text-detail text-gray600">{{ education.status }}</span>
      </div>
    </div>
  </div>
</template>
